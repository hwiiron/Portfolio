...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function setActiveNavLink() {
    ...
    sections.forEach(function (section) {
      ...
      var targetNavLink = document.querySelector(
        'nav ul li a[href="#' + section.id + '"]'
      );
      if (
        (currentScroll >= sectionTop &&
          currentScroll < sectionTop + sectionHeight) ||
        currentScroll + window.innerHeight >= document.body.offsetHeight
      ) {
...
$(window).on("scroll", function () {
  ...
  if (windowBottom >= footerTop) {
    setTimeout(function () {
      $(
        "#contact .inner .communication dt, #contact .inner .communication dd"
      ).addClass("on");
    }, 500);
